<script lang="ts">
	...
	import DataTable, { type ColumnInfo, type DataRow } from './components/DataTable/DataTable.svelte';
	...
	async function loadParquetFile(filePath: string) {
		...
		// Resetar estado ao abrir novo arquivo
		schema = [];
		rows = [];
		sortCol = null;
		sortOrder = null;
	...
	// Gerencia o clique no cabeçalho da tabela
...
<main class="container">
	...
	<div class="content">
		...
		{#if !hasData && !errorMsg}
			<div class="empty-state-container">
				...
				<p class="welcome-subtitle">Arraste um arquivo ou clique abaixo para começar.</p>

				<button class="btn-primary large-btn" on:click={handleOpenFile} disabled={isLoading}>
					{#if isLoading}
		...
		{#if errorMsg}
			<div class="actions-bar">
				...
				<button class="btn-primary" on:click={handleOpenFile}>Tentar Outro Arquivo</button>
			</div>
		{/if}
	...
	{#if showAboutModal}
		<Modal title={`Sobre o ${APP_INFO.name}`} onclose={() => showAboutModal = false}>
			<div class="about-content">
				...
				<p style="font-size: 1.2rem; font-weight: 700; margin-bottom: 0.2rem;">
                    {APP_INFO.name} <span style="font-weight:400; opacity: 0.7;">{APP_INFO.version}</span>
                </p>
				
				<a 
					href={APP_INFO.social.githubRepo} 
					class="repo-link" 
					on:click|preventDefault={() => openExternal(APP_INFO.social.githubRepo)}
				>
				...
				<div class="social-links">
					<a 
						...
						on:click|preventDefault={() => openExternal(APP_INFO.social.githubProfile)}
					>
					...
					<a 
						...
						on:click|preventDefault={() => openExternal(APP_INFO.social.linkedin)}
					>
					...
					<a 
						...
						on:click|preventDefault={() => openExternal(APP_INFO.social.email)}
					>
				...
				<a 
					...
					class="attribution" 
					on:click|preventDefault={() => openExternal(APP_INFO.attribution.storyset.url)}
				>
