<script lang="ts">
	...
	let selectedScope = $state<ExportScope>('all');

	$effect(() => {
		if (isOpen) {
			selectedScope = isSqlMode ? 'query' : 'all';
		}
	});
...
{#if isOpen}
	<Modal title="Exportar Dados" {onclose}>
		<div class="export-options">
			<div>
				...
				<div class="format-grid">
					...
					<button 
						...
						onclick={() => selectedFormat = 'PARQUET'}
					>
			...
			<div class="actions">
				<button class="btn-secondary" onclick={onclose} disabled={isLoading}>Cancelar</button>
				<button class="btn-primary" onclick={handleExport} disabled={isLoading}>
					{#if isLoading}
